...
import { Upload, Trash2, Database, RefreshCw, FileText, Github } from 'lucide-react'
...
import { importGitHubRepository, GitHubImportOptions } from '@/services/rag/githubImporter'
...
export const KnowledgeManager: React.FC<KnowledgeManagerProps> = ({ isOpen, onClose }) => {
  ...
  const handleGitHubImport = async (options: GitHubImportOptions) => {
    setIsIndexing(true)
    ...
    try {
      ...
      if (!gitServer) {
        throw new Error('GitHub MCP server (git) is not connected. Please connect it first.')
      }

      const { texts, metadatas } = await importGitHubRepository(
        mcpService,
        gitServer.id,
        options,
        (status, current, total) => {
          setProgress(`${status} ${total > 0 ? `(${current}/${total})` : ''}`)
        },
      )

      if (texts.length === 0) {
        throw new Error('No files were imported. Check your filters or repository.')
      }

      setProgress(`Building index for ${texts.length} files...`)
      ...
      saveIndex(newIndex)
      setIndex(newIndex)
      setProgress(`Successfully imported ${texts.length} files!`)
  ...
  return (
    <>
        ...
        <DialogContent>
          <Box sx={{ mb: 3 }}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
              ...
              {index && (
                <Button size="small" startIcon={<FileText size={16} />} onClick={handleExportIndex}>
                  Export JSON
                </Button>
              )}
            </Box>
          ...
          <Box sx={{ mb: 4 }}>
            ...
            <Box sx={{ display: 'flex', gap: 2, mb: 2 }}>
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 1 }}>
                ...
                <Button variant="outlined" onClick={handleOpenImport} startIcon={<RefreshCw />} size="small">
      ...
      <GitHubImportDialog
        ...
        onClose={() => !isIndexing && setIsGitHubImportOpen(false)}
